    ...
    try:
        ...
        if pattern_type == "meal":
            ...
            for event_time in event_times:
                ...
                if len(window) >= 36:  # Ensure at least 3 hours (36 * 5min intervals)
...
def create_diary(df, date):
    try:
        ...
        total_time = len(daily_df) * 5 / 60  # Convert 5-min intervals to hours
...
def generate_insights(df, date, hypo_threshold=70, start_date=None, end_date=None, goal_range=None, profile=None):
    try:
        ...
        if len(date_range) > 1:
            ...
            if trend > 20:
                recommendations.append(html.P("Recommendation: Consider increasing insulin dosage to manage rising glucose levels."))
            elif trend < -20:
                recommendations.append(html.P("Recommendation: Consider increasing carb intake or reducing insulin to address falling glucose levels."))

        # Anomalies with profile-based adjustments
        ...
        if hypo_events > 0:
            if activity_level == "high":
                ...
                carb_adjust = 25
            recommendations.append(html.P(f"Recommendation: Increase carb intake by ~{carb_adjust-5}-{carb_adjust}g or decrease insulin by ~1-2U to prevent {hypo_events} hypoglycemia (<{hypo_threshold} mg/dL) event(s)."))
        if hyper_events > 0:
            if activity_level == "high":
                insulin_adjust = 1.5
            elif activity_level == "moderate":
                ...
                insulin_adjust = 0.5
            recommendations.append(html.P(f"Recommendation: Increase insulin by ~{insulin_adjust}-{insulin_adjust+0.5}U or reduce carb intake by ~15-30g to address {hyper_events} hyperglycemia (>250 mg/dL) event(s)."))
        ...
        if not date_range[date_range["carbs"] > 0].empty and not date_range[date_range["bolus_insulin"] > 0].empty:
            ...
            if abs(carb_glucose_corr) > 0.5:
                recommendations.append(html.P(f"Recommendation: Adjust carb intake based on a correlation of {carb_glucose_corr:.2f} with glucose changes."))
            if abs(insulin_glucose_corr) > 0.5:
                recommendations.append(html.P(f"Recommendation: Adjust insulin dosage based on a correlation of {insulin_glucose_corr:.2f} with glucose changes."))

        # General Balance
        if mean_glucose < 70:
            recommendations.append(html.P("Recommendation: Increase overall carb intake or reduce insulin to maintain glucose in the 70-180 mg/dL range."))
        elif mean_glucose > 180:
            recommendations.append(html.P("Recommendation: Increase insulin or decrease carb intake to maintain glucose in the 70-180 mg/dL range."))
        ...
        if start_date and end_date:
            ...
            if days > 1:
                recommendations.append(html.P(f"Summary: Over {days} days, {total_hypo} hypoglycemia and {total_hyper} hyperglycemia events detected. Consider reviewing insulin or carb adjustments with a healthcare provider."))

        # Goal Tracker with profile-based adjustment
        if goal_range and len(goal_range) == 2 and all(x is not None for x in goal_range):
            ...
            if tir_goal < 70:
                ...
                if activity_level == "high":
                    ...
                    insulin_adjust = 0.7
                    carb_adjust = 15
                recommendations.append(html.P(f"Goal Tracker: Only {tir_goal:.1f}% time in {goal_min}-{goal_max} mg/dL range. Increase insulin by ~{insulin_adjust}U or reduce carbs by ~{carb_adjust}g to improve by ~{gap:.1f}%."))
        ...
        if len(date_range) > 10:
            ...
            if predicted_glucose < 70:
                recommendations.append(html.P(f"Predictive Insight: Glucose may drop to ~{predicted_glucose:.0f} mg/dL tomorrow. Consider increasing carbs by ~10g."))
            elif predicted_glucose > 180:
                recommendations.append(html.P(f"Predictive Insight: Glucose may rise to ~{predicted_glucose:.0f} mg/dL tomorrow. Consider increasing insulin by ~0.5U."))
...
# Layout
app.layout = html.Div([
    html.H1("DiaView: Type 1 Diabetes Visualization", style={"textAlign": "center"}),
    dcc.Tabs(id="tabs", value="visualizations", children=[
        ...
        dcc.Tab(label="Diary", value="diary", children=[
            html.Div([
        ...
        dcc.Tab(label="Insights", value="insights", children=[
            html.Div([
                html.Label("Select Day for Insights:"),
                dcc.Dropdown(id="insight-date-dropdown", options=[{"label": date, "value": date} for date in unique_dates], 
                             value=default_detail_date_diary, multi=False),
                html.Label("Select Date Range for Summary:"),
                dcc.DatePickerRange(
                ...
                html.Label("Hypoglycemia Threshold (mg/dL):"),
                dcc.Dropdown(id="hypo-threshold-dropdown", options=[{"label": str(th), "value": th} for th in hypo_threshold_options], 
                             value=70, multi=False),
                html.H3("Goal Range (mg/dL):"),
                html.Div([
                    dcc.Input(id="goal-min", type="number", value=70, min=40, max=300, style={"margin": "5px", "width": "100px"}),
                    dcc.Input(id="goal-max", type="number", value=180, min=40, max=300, style={"margin": "5px", "width": "100px"})
                ], style={"display": "flex", "justifyContent": "center"}),
                html.H3("User Profile", style={"textAlign": "center", "margin": "10px"}),
                html.Div([
                    html.Label("Age:"), dcc.Input(id="age-input", type="number", value=30, min=1, max=120, style={"margin": "5px"}),
                    html.Label("Weight (kg):"), dcc.Input(id="weight-input", type="number", value=70, min=1, max=200, style={"margin": "5px"}),
                    html.Label("Activity Level:"), dcc.Dropdown(id="activity-level-dropdown", options=[{"label": al, "value": al} for al in activity_level_options], value="moderate", style={"margin": "5px"})
                ], style={"display": "flex", "flexDirection": "column", "alignItems": "center"}),
                html.Div(id="insights-content")
            ], style={"padding": "20px"})
        ]),
        dcc.Tab(label="Learn", value="learn", children=[
            html.Div([
